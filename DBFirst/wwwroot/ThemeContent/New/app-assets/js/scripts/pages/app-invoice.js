    ...
    if (datepicker.length) {
      /*  datepicker.each(function () {
        ...
        /*    debugger;
            ...
            if (valuedate != "" && valuedate != null && valuedate != undefined) {
                $(this).flatpickr({ defaultDate: valuedate });
            } else {
                $(this).flatpickr({
    ...
    if (btnAddNewItem.length) {
        btnAddNewItem.on('click', function () {
            if (feather) {
               ...
               // setTimeout(function () { select2CustomDestroyCreate(); }, 1);
                 featherSVG();
...
function select2CustomDestroyCreate() {


    /*var select2items = $('select .select2');*/
   clearTimeout(function () { select2CustomDestroyCreate(); }, 1);


    var aElems = document.getElementsByTagName("select");
    for (var i = 0; i < aElems.length; ++i) {
        var classesArr = aElems[i].className.split(/\s+/),
            classesObj = {};
        for (var j = 0; j < classesArr.length; ++j) {
            classesObj[classesArr[j]] = true;
        }
       
        if (classesObj.hasOwnProperty("select2")) {
            // action
            try {
                $(aElems[i]).select2('destroy');
            } catch (Exception) {
                var vvvvvvvvvvvvvvv = 0;
            }
            $(aElems[i]).removeClass('select2');
    ...
    for (var k = 0; k < aElems.length; ++k) {
        ...
        $(aElems[k]).addClass('select2');
    }
    $('.select2').select2();
   /* debugger;
    for (var i = 0; i < select2items.length; i++) {
        try {
           
            $(select2items[i]).select2('destroy');
        } catch (Exception) { var v = 0; }
    }*/
